...
for ep in range(episodes):
    ...
    while not done:
        ...
        if len(buffer) >= 1_000:
            ...
            with torch.no_grad():
                a_star = policy_net(S2).argmax(1, keepdim=True) # 선택
                q_next = target_net(S2).gather(1, a_star).squeeze() # 평가
    ...
    DDQN_returns.append(ep_ret)
    if (ep+1) % 10 == 0:
        print(f"Ep {ep+1:4d} | Return: {ep_ret:3.0f} | ε={eps:.3f}")

print("평균 리턴(최근100):", np.mean(DDQN_returns[-100:]))
np.save("returns_ddqn.npy", DDQN_returns)
torch.save(policy_net.state_dict(), "ddqn_cartpole.pth")

DDQN_returns = np.load("returns_ddqn.npy")
plt.plot(DDQN_returns, alpha=.3)
plt.plot(np.convolve(DDQN_returns, np.ones(100)/100, 'valid'))
plt.title("CartPole DDQN Return")
plt.savefig("return_curve_ddqn.png")
